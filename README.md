...
> 本仓库是 **MoonBit 基础设施方向练手场**：包含 10 个基础设施级方向的骨架实现（parser_combinator / regex_engine / serialization / build_tool / logging / codegen_infra / dst / lsp_binding+lsp_server / mini_compiler / actor），
> 以 [moonbit-pathfinding](https://github.com/Suquster/moonbit-pathfinding) 图算法库为公共底座（build_tool 的拓扑调度等直接复用 `@directed`）。
> 内容源自 moonbit-pathfinding PR #5，因与参赛项目主题无关而独立成库，作为下一步钻研的起点。
> 各方向规划见 `.kiro/specs/moonbit-infra-suite/`。703 个测试全部通过（wasm-gc / js / native）。
